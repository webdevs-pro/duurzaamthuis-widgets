	...
	if ( 'dh-product-comparition-sustainability-score' === $widget->get_name() ) {
		...
		foreach ( $i_tags as $i_tag ) {
			...
			if ( strpos( $i_tag_classes, 'dh-icon' ) !== false ) {
				...
				$svg = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="' . ( $paths[$icon_name]['width'] ?? '' ) . '" height="' . ( $paths[$icon_name]['height'] ?? '' ) . '" viewBox="0 0 1024 1024">' . ( $paths[$icon_name]['path'] ?? '' ) . '</svg>';
