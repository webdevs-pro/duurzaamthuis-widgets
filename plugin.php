	...
	if ( 'dh-product-comparition-sustainability-score' === $widget->get_name() ) {
		...
		foreach ( $i_tags as $i_tag ) {
			...
			if ( strpos( $i_tag_classes, 'dh-icon' ) !== false ) {
				$icon_name = str_replace( 'dh-icon dh-icon-', '', $i_tag_classes );
				
				$svg = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 1024 1024">' . ( $paths[$icon_name] ?? '' ) . '</svg>';
