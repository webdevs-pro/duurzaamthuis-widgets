...
add_filter( 'elementor/utils/get_the_archive_title', 'duurza_archive_title' );
function duurza_archive_title( $title ) {
	if ( is_search() ) {
		return 'Zoekresultaten: ' . get_search_query();
	}
	return $title;
}


add_filter( 'dfrcs_products', function( $filtered_products, $all_products ) {
	error_log( "filtered_products\n" . print_r($filtered_products, true) . "\n" );
